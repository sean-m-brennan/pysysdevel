    ...
    def run(self):
        ...
        if self._has_fortran_tests():
            ...
            for pkg, units in self._get_fortran_tests():
                for unit in units:
                    unit.sources += fortran_unittest_framework
        ...
        if self._has_cpp_tests():
            raise Exception("C/C++ unit testing not yet supported.") #FIXME
            ...
            for pkg, units in self._get_fortran_tests():
                for unit in units:
                    unit.sources += util.create_cppunit_driver()
            ...
            for pkg, units in self._get_fortran_tests():
                log.info('C/C++ unit tests for ' + pkg)
                for unit in units:
                    util.check_call([os.path.join(lib_dir, unit.name)])
